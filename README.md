...
![Insitu-Photos Preview](https://via.placeholder.com/1200x600?text=Insitu-Photos+Preview)
...
1.  **Clone the repository:**
    ```bash
    git clone https://github.com/ysapozhnikov/insitu-photos.git
    cd insitu-photos
    ```
...
1.  Ensure you have the **NVIDIA Container Toolkit** installed via the Unraid Community Applications.
2.  Map your photo share as a **Read-Only** volume in `docker-compose.yml`:
    ```yaml
    volumes:
      - /mnt/user/photos:/photos:ro
    ```
3.  The application will generate thumbnails and store metadata in the `./cache` volume.
